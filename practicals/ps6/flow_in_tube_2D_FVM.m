...
unwall = 0.;            % north side velocity [m/s]
uswall = 0.;
vewall = 0.;
vwwall = 0.;

% [INOUT] Inlet velocities
uin = 1.;
...
for m=1:nsteps
    ...
    u(1,:) = uin;               % fixed inlet velocity
    
    % [INOUT] Over-writing outlet conditions
    u(nx+1,:) = u(nx,:);    % zero-gradient outlet velocity
    v(nx+2,:) = v(nx+1,:);  % zero-gradient outlet velocity
    ...
    ut(1,:) = u(1,:);            % fixed inlet velocity
    ut(nx+1,:) = u(nx+1,:);  % zero-gradient outlet velocity
    vt(nx+2,:) = v(nx+2,:);  % zero-gradient outlet velocity
