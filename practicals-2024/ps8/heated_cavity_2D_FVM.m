...
for m=1:nsteps
    ...
    T(1,:) = 2*Tcold - T(2,:);              % west wall
    T(end,:) = 2*Thot - T(end-1,:);         % east wall
    T(:,1) = T(:,2);                        % north wall
    T(:,end) = T(:,end-1);                  % south wall
  ...
  %-- [HEATED] On-the-fly post-processing
...
ref_u_along_y = readmatrix ("benchmark-solution");
