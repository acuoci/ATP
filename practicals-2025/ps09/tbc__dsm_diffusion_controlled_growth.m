...
% Plot solution
figure; hold on;
plot(r, fIn, 'g', 'LineWidth',2);
plot(r, f(end,:), 'r', 'LineWidth',2);
...
fprintf('Time=0 s:  Ntot(#/cm^3)=%f Rm(micron)=%f\n', m0(1), m0(2));
fprintf('Time=20 s: Ntot(#/cm^3)=%f Rm(micron)=%f\n', mf(1), mf(2));
...
for k=1:length(t)
     hold off;
     plot(r, fAnalytical(r, t(k), kG, a, b), 'r', 'LineWidth',2);
     hold on;
     plot(r, f(k,:), 'b--', 'LineWidth',2);
     hold on;
     xlabel('r (\mum)'); ylabel('f (#/micron/cm^3)');
