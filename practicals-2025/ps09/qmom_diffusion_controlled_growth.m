...
for j=1:length(r)-1
    deltar = r(j+1)-r(j);
    for k=0:Neq-1
        Ic = 0.50*(r(j+1)^(k)*fIn(j+1)+r(j)^(k)*fIn(j));
        muIn(k+1) = muIn(k+1) + Ic*deltar;
    end
end
...
figure;
tiledlayout(2,3);
nexttile; plot(t, muNorm(:,1), 'LineWidth',2); xlabel('time (s)'); ylabel('\mu_0^{norm}');
nexttile; plot(t, muNorm(:,2), 'LineWidth',2); xlabel('time (s)'); ylabel('\mu_1^{norm}');
nexttile; plot(t, muNorm(:,3), 'LineWidth',2); xlabel('time (s)'); ylabel('\mu_2^{norm}');
nexttile; plot(t, muNorm(:,4), 'LineWidth',2); xlabel('time (s)'); ylabel('\mu_3^{norm}');
nexttile; plot(t, muNorm(:,5), 'LineWidth',2); xlabel('time (s)'); ylabel('\mu_4^{norm}');
nexttile; plot(t, muNorm(:,6), 'LineWidth',2); xlabel('time (s)'); ylabel('\mu_5^{norm}');
...
plot(r, fIn, 'b', 'LineWidth',2);
[w, L] = MomentInversion(muNorm(1, :));
for i=1:N
    line([L(i),L(i)], [0 w(i)*muIn(1)], 'LineWidth',2, 'Color','r');
end
...
plot(r, fIn, 'g');
plot(r, fAnalytical(r, tf, kG, a, b), 'b', 'LineWidth',2);
[w, L] = MomentInversion(muNorm(end, :));
for i=1:N
    line([L(i),L(i)], [0 w(i)*muIn(1)], 'LineWidth',2, 'Color','r');
end
legend('initial', 'analytical','numerical');
hold off;
...
for k=1:length(t)
     ...
     hold off;
     plot(r, fAnalytical(r, t(k), kG, a, b), 'b', 'LineWidth', 2);
     hold on;
     for i=1:N
         line([L(i),L(i)], [0 w(i)*muIn(1)], 'LineWidth',2, 'Color','r');
     end
