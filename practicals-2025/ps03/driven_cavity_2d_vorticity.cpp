...
int main()
{
	...
	const double tau = 20;								// total time of simulation[-]
	...
	for (unsigned int istep = 1; istep <= nsteps; istep++)
	{
		...
		if (poisson_sor_solver == true)
		{
			for (iter = 1; iter <= max_iterations; iter++)
			{
				...
				for (unsigned int i = 1; i < nx-1; i++)
					for (unsigned int j = 1; j < ny-1; j++)
						epsilon += std::abs( ( psi(i+1,j) - 2.*psi(i,j) + psi(i-1,j) )/h/h + 
											 ( psi(i,j+1) - 2.*psi(i,j) + psi(i,j-1) )/h/h + 
											   omega(i,j)  );
											   
				epsilon /= static_cast<double>((nx-2)*(ny-2))
