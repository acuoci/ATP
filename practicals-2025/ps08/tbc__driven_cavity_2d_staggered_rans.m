...
kappa0=0;           % [RANS] initial value of turbulent kinetic energy [m2/s2]
...
% [RANS] Boundary conditions for turbulent kinetic energy
...
for is=1:nsteps
    ...
    % [RANS] Update the turbulent viscosity (Prandtl k-l model)
    ...
    % [RANS] Boundary conditions (Turbulent kinetic energy)
    ...
    % [RANS] Update passive scalar solution
    ...
    if (mod(is,200)==1)
        ...
        subplot(234);
        contourf(X,Y,kk',20,'LineColor','none');
        axis('square'); title('turb. kin. energy k (m^2/s^2)');
        colormap('jet'); colorbar;
        
        % Contour: turbulent viscosity
        subplot(235);
        contourf(X,Y,nutnut',20,'LineColor','none');
        axis('square'); title('turb. viscosity (m^2/s)');
        colormap('jet'); colorbar;
...
% --------------------------------------------------------------------------------------
% [RANS] Advection-diffusion equation for turbulent kinetic energy
% --------------------------------------------------------------------------------------
